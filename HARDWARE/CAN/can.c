...
{		
#if 1
		uint32_t priority = 0x00;						//uavcan 优先级
	  uint32_t messagetypeID;							//uavcan message ID（）
	  uint32_t servicenotmessage = 0x00;	//uavcan  message传输方式 service设置为0
    uint32_t sourcenodeID = 0x01;				//source  node ID
		CanTxMsg  TxMessage;
	  
		messagetypeID = device_id;
		TxMessage.ExtId = ((priority << 24)|(messagetypeID <<8) | (servicenotmessage <<7 )| sourcenodeID );//uavcan 的ID field
